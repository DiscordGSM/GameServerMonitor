...
class ServerConfig:
    ...
    def process_auth_params(game_id: str, auth_params: Dict) -> Dict:
        ...
        elif game_id == "scpsl" and all(
            k in auth_params for k in ["account_id", "api_key"]
        ):
            query_extra["_api_key"] = auth_params["api_key"]
            return {
                "address": auth_params["account_id"],
                "query_port": 0,
        ...
        elif game_id == "tmnf" and all(
            k in auth_params for k in ["username", "password"]
        ):
            query_extra.update(
                {
...
class DGSMAutomation:
    ...
    def server_exists(
        self, address: str, query_port: int
        ...
        with sqlite3.connect(self.db_path) as conn:
            ...
            if result:
                return True, {"channel_id": result[0], "game_id": result[1]}
            return False, None

    def add_server(self, config: ServerConfig) -> Tuple[bool, str]:
        try:
            exists, existing_data = self.server_exists(
                config.address, config.query_port
            )
            if exists:
                return (
                    False,
                    f"Server {config.address}:{config.query_port} already exists in channel {existing_data['channel_id']} for game {existing_data['game_id']}",
                )
            ...
            with sqlite3.connect(self.db_path) as conn:
                ...
                return (
                    True,
                    f"Successfully added server {config.address}:{config.query_port}",
                )
...
def main():
    ...
    parser.add_argument(
        "--ignore-existing", action="store_true", help="Continue if server exists"
    )

    # Style configuration
    parser.add_argument(
        "--style", choices=VALID_STYLES, default="Medium", help="Message style"
    )
    ...
    parser.add_argument(
        "--locale", type=str, default="en-US", help="Locale for translations"
    )
    parser.add_argument(
        "--timezone", type=str, default="UTC", help="Timezone for timestamps"
    )
    parser.add_argument(
        "--clock_format", choices=["12", "24"], default="12", help="Clock format"
    )
