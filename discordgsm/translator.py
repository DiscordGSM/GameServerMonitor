...
def t(locale_str: str, locale: Locale):
    ...
    return translations.get(str(locale), translations["en-US"]).get(
...
class Translator(app_commands.Translator):
    ...
    async def translate(
        ...
        return self.translations.get(locale.value, self.translations["en-US"]).get(
            locale_str.message, locale_str.message
        )
...
if __name__ == "__main__":
    ...
    subparsers.add_parser(
        "update", description="Update all locale json files base on en-US.json"
    )
    ...
    if args.subparser_name == "update":
        with open(os.path.join(path, "translations", "en-US.json"), "rb") as f:
            enUS: OrderedDict[str, str] = json.loads(
