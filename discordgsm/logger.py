...
log_path = os.path.join(
    os.path.dirname(os.path.realpath(__file__)), "..", "data", "logs"
)
Path(log_path).mkdir(parents=True, exist_ok=True)

# Set up TimedRotatingFileHandler
file_handler = TimedRotatingFileHandler(
    os.path.join(log_path, "discordgsm.log"), when="D", encoding="utf-8"
)
...
logger.setLevel(
    logging.DEBUG if os.getenv("APP_DEBUG", "").lower() == "true" else logging.INFO
)
